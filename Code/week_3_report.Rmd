...
```{r substance use treatment centers in va}
...
treatment_centers$sa <- as.integer(treatment_centers$sa)
treatment_centers_ggplot <- ggplot(data = world) +
  ...
  labs(fill= "Mortality Rate 
Per 100,000 People")+
  geom_sf(data = treatment_sf, aes( color = "red")) +
  guides(color= guide_legend(title= "Treatment Centers")) +
  coord_sf(xlim = c(-84, -75), ylim = c(36, 40), expand = FALSE) +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("Substance Abuse Treatment Centers and Drug Overdose Rates in 2023") +
  theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.5),
        panel.background = element_rect(fill = "azure1"))
