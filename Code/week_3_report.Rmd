...
#Map Built-in
vc_path <- "~/DSPG/Repose/DSPG_VCE_Health/Data/virginia_counties.csv"
vc <- read.csv(vc_path, header= TRUE)
...
# Treatment Center Location Data
...
ranked <- ranked23_measured_data %>%
  select(County,FIPS,`% Vaccinated`, `# Mental Health Providers`, `Dentist Rate`, 
...
```
```{r vaccine}
vac_2023 <- ranked23_counties %>%
  select(County,GEOID,`% Vaccinated`)

names(vac_2023)[3] = "Per_Vaccinated"
most_phys <- vac_2023 %>%
  arrange(desc(Per_Vaccinated)) %>%
  slice_min(Per_Vaccinated, n= 3) %>%
  print()
...
saveRDS(states, file = "us.states.Rdata")
...
(Source: Virgnia Department of Health)", 
...
```{r substance use treatment centers/vce agents/ mortality rate}
#treatment center locations set-up
treatment_sf <- st_as_sf(treatment_centers, coords = c("longitude", "latitude"),
                        remove =FALSE, crs = 4326, agr = "constant")

treatment_centers_ggplot <- ggplot(data = world) +
  geom_sf(data = states, color= "grey60", fill= "ivory1") +
  geom_sf(data = counties, fill = NA) +
  geom_sf(data= va.drug, aes(fill= Rate_Drug_Overdose_Deaths))+
  scale_fill_viridis_c(trans= "sqrt", alpha= .4, direction= -1) +
  labs(fill= "Mortality Rate 
Per 100,000 People")+
  guides(color= guide_legend(title= "Sites")) +
  geom_sf(data = treatment_sf, aes(color= "Substance Abuse
  Treatment Center")) +
  geom_sf(data = agents_sf, aes(color = "FCS Agents"), size= 3) +
  coord_sf(xlim = c(-84, -75), ylim = c(36, 40), expand = FALSE) +
  xlab("Longitude") + ylab("Latitude") +
  ggtitle("FCS Agents and Substance Abuse Treatment Centers", 
          subtitle= "Drug Overdose Mortality Rate 2018-2020") +
  labs(caption = "Source: County Health Rankings 2023") +
  theme(panel.grid.major = element_line(color = gray(0.5), linetype = "dashed", size = 0.5),
        panel.background = element_rect(fill = "azure1"))
treatment_centers_ggplot 
```
