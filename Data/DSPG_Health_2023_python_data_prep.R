...
z_obese_long <- z.obese
for (i in 2:36) {z_obese_long <- c(z_obese_long, z.obese)}

z_diabetes_long <- z.diabetes
for (i in 2:36) {z_diabetes_long <- c(z_diabetes_long, z.diabetes)}

z_food_long <- z.food
for (i in 2:36) {z_food_long <- c(z_food_long, z.food)}

z_inactive_long <- z.inactive
for (i in 2:36) {z_inactive_long <- c(z_inactive_long, z.inactive)}

z_low_bw_long <- z.low.bw
for (i in 2:36) {z_low_bw_long <- c(z_low_bw_long, z.low.bw)}
...
long_df <- as.data.frame(cbind(a_long, c_long, d_long, p_long, z_long, z_obese_long, z_diabetes_long, z_food_long, z_inactive_long, z_low_bw_long))

write.csv(long_df, 'C:/Users/vivia/Documents/DSPG/Repose/DSPG_VCE_Health/Data/program_data.csv', row.names = FALSE)
...
z_obese_long <- z.obese
for (i in 2:75) {z_obese_long <- c(z_obese_long, z.obese)}

z_diabetes_long <- z.diabetes
for (i in 2:75) {z_diabetes_long <- c(z_diabetes_long, z.diabetes)}

z_food_long <- z.food
for (i in 2:75) {z_food_long <- c(z_food_long, z.food)}

z_inactive_long <- z.inactive
for (i in 2:75) {z_inactive_long <- c(z_inactive_long, z.inactive)}

z_low_bw_long <- z.low.bw
for (i in 2:75) {z_low_bw_long <- c(z_low_bw_long, z.low.bw)}
...
a_long <- c()
for (i in 1:75) {a_long <- c(a_long, rep(new$County[i], 133))}

long_df <- as.data.frame(cbind(a_long, c_long, d_long, p_long, z_long, z_obese_long, z_diabetes_long, z_food_long, z_inactive_long, z_low_bw_long))

write.csv(long_df, 'C:/Users/vivia/Documents/DSPG/Repose/DSPG_VCE_Health/Data/program_data_new.csv', row.names = FALSE)
